function V = sample_model_given_chgpt_d18O(start, stop,XX,Y,parameters)
...
% this function and samp_model_given_chgpt_sim.m is that here there are less 
% than 2^m subsets of predictor variables to consider.
%
...
function PROB = tree(invG, detG, Am, C)
        ...
        if (C < m)
            ...
            if(M1+M2+M3+Am(7)-Am(9)<2)     % No conflicting models
                ...
                P2=zeros(1,2^(m-C))-Inf;   % Accounts for non-permissible sub-models.  Assigns them log probability -Inf
            end
            PROB=[PROB P2];
            ...
            else P2= -Inf;                  % Non-permissible models are given a log probability of -Inf
